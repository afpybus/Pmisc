...
```{r create_binary_data, warning=FALSE}
...
```{r upset_text_size, warning=FALSE}
...
```{r side_panel_left, warning=FALSE}
# Left side panel
upset_side(mtcars_binary, side = "left")
```

```{r side_panel_right, warning=FALSE}
...
```{r combined, fig.width=10, fig.height=5, warning=FALSE}
# Create both plots
...
```{r custom_values_text, warning=FALSE}
...
```{r side_expansion, warning=FALSE}
...
```{r custom_styling, warning=FALSE}
