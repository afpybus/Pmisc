...
- Test multiple features for association with survival using Cox
...
These functions provide a streamlined wrapper around the **`survival`**,
**`tidycmprsk`**, and **`ggsurvfit`** packages.

This vignette demonstrates these functions using the `lung` dataset from
...
The `lung` dataset contains survival data from patients with advanced
lung cancer:
...
The Pmisc survival functions expect `status` coded as 0/1 (0 = censored,
...
The `coxph_all()` function tests multiple features simultaneously for
...
| feature    |   n | hazard_ratio | wald_p |  p.adj | p.signif | result           |
|:-----------|----:|-------------:|-------:|-------:|:---------|:-----------------|
...
- **Hazard ratio \> 1**: Higher feature values associated with increased
  risk of death
- **Hazard ratio \< 1**: Higher feature values associated with decreased
  risk of death
...
- **ph.ecog** shows increased hazard (worse performance status = worse
  survival)
...
![](figures/survival_analysis-survival_volcano-1.png)
...
You can control the exact colors of the volcano plot using the `scm()`
helper:
...
![](figures/survival_analysis-custom_volcano-1.png)

## Kaplan-Meier Survival Curves

The `KM_categorical()` function creates Kaplan-Meier curves for
...
![](figures/survival_analysis-km_sex-1.png)

### Continuous Variable: Age

For continuous variables, `KM_categorical()` automatically splits at the
median:
...
![](figures/survival_analysis-km_age-1.png)
...
![](figures/survival_analysis-km_ecog-1.png)

## Gene Expression vs Overall Survival

The `gene_OS_scatter()` function creates scatter plots showing the
relationship between a feature and survival time:
...
![](figures/survival_analysis-gene_os_scatter_wt-1.png)
...
| feature  | hazard_ratio |  p.adj | result           |
|:---------|-------------:|-------:|:-----------------|
...
![](figures/survival_analysis-complete_workflow-1.png)

## Advanced: Custom Survival Analysis

For more complex analyses, you can combine Pmisc functions with base
survival functions:
...
#> Score (logrank) test = 16.91  on 5 df,   p=0.005
```
...
- Female sex (HR ≈ 0.59): Females have ~41% lower risk of death than
  males
- ECOG score (HR ≈ 1.62): Each 1-point increase in ECOG score increases
  risk by ~62%
