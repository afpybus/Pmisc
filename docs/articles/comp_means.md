...
The `comp_means()` function in Pmisc provides a streamlined way to
perform **pairwise statistical comparisons** across multiple features
**between two groups**. It automatically handles:
...
The companion function `volcano_cm()` creates volcano plots to visualize
the results:
...
Use `DEF_boxplot()` to visualize specific features with p-value
annotations:
...
Similar to `scm()`, the `sfm()` (Scale Fill Manual) helper allows you to
easily apply custom fill colors using the same tibble format.
