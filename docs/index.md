...
Personal R package containing utility functions for data analysis,
visualization, statistics, survival analysis, and more.
...
- **Utility Functions**: `add_sig()`, `lu()`, `tibble_to_mat()`,
  `slope()`, `earliest()`, `latest()`
- **Figure Helpers**: `scm()`, `sfm()`, `geom_vb()`, `title_format()`,
  `bar.counts()`, `make_grid()`, `png_pdf()`, `ggplotColours()`
- **Data Manipulation**: `rm_from_array()`, `gsub_colnames()`,
  `get_upper_tri()`, `get_lower_tri()`, `reorder_cormat()`
- **Stat Functions**: `comp_means()`, `p.readjust()`, `volcano_cm()`,
  `corr_across()`, `remove_outliers()`, `DEF_boxplot()`
- **Spatial Analysis**: `make_pairs()`
- **Survival Analysis**: `coxph_all()`, `survival_volcano()`,
  `KM_categorical()`, `gene_OS_scatter()`
